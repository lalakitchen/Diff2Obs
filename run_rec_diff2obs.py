...
This CLI can recover the baseline experiments with Diff2Obs defense support.
"""
import os
...
# Import Diff2ObsDefense class directly
from defenses.diff2obs import Diff2ObsDefense
...
def apply_additive_noise(input_gradient, std=0.01):
    """Injects additive Gaussian noise to private gradients."""
    if std is None or std <= 0:
        return input_gradient
    return tuple(g + torch.randn_like(g) * std for g in input_gradient)


nclass_dict = {
    'I32': 1000, 'I64': 1000, 'I128': 1000, 
...
parser.add_argument('--checkpoint_path', default='', type=str, help='Pretrained generator checkpoint path')
parser.add_argument('--gan', default='stylegan2', type=str, help='GAN model option:[stylegan2, biggan]')
parser.add_argument('--config', default='./configs_stylegan.yml', type=str, help='Path of selected config file.')
...
if __name__ == "__main__":
    ...
    # Change the exp path
    config['exp_name'] = f"{config['exp_name']}_num_images_{config['num_images']}_defense_{config['defense_method']}_{current_time}"
    ...
    logger.info(f"output_dir: {save_dir}")

    setup = inversefed.utils.system_startup(args)

    save_experiment_config(config_path, save_dir)
    logger.info("Config for this experiment is saved")
    ...
    if isinstance(set_seed, int):
        logger.info(f"Set seed:{set_seed}")
    ...
    if config['dataset'].startswith('FFHQ') or config['dataset'].endswith('FFHQ'):
        dm = torch.as_tensor(getattr(inversefed.consts, 'cifar10_mean'), **setup)[:, None, None]
        ds = torch.as_tensor(getattr(inversefed.consts, 'cifar10_std'), **setup)[:, None, None]
    else:
    ...
    optimizer = torch.optim.SGD(model.parameters(), lr=defs.lr, momentum=0.9, weight_decay=defs.weight_decay)
    ...
    for epoch in range(config['train_epochs']):
        ...
        for i in range(config['num_exp']): 
            ...
            if args.accumulation == 0:
                logger.info(f"Ground truth's size:{ground_truth[0].shape}")
                ...
                inversefed.utils.save_to_table(
                    ...
                    name=f'{epoch}_epoch_input_gradient_norm', 
                    dryrun=args.dryrun, 
                    input_gradient_norm=input_gradient_tmp, 
                ...
                if config['defense_method'] is None or str(config['defense_method']).lower() in ['none', 'no_defense']:
                    ...
                    with torch.no_grad():
                        synth_label = diff2obs_module.select_label_agnostic(model, ground_truth[0:1], labels[0:1])
                        synth_labels = torch.full_like(labels, fill_value=synth_label)
                        x_synth = torch.randn_like(ground_truth) # Candidate synthetic input

                    model.zero_grad()
                    synth_loss, _, _ = loss_fn(model(x_synth), synth_labels)
                    grad_synth = torch.autograd.grad(synth_loss, model.parameters())

                    # Apply obfuscation to gradients
                    mode = defense_setting.get('mode', 'Obs-I')
                    obfuscated_grads = []
                    for gp, gs in zip(input_gradient, grad_synth):
                        go = diff2obs_module.obfuscate_gradients(gp, gs, round_r=epoch, strategy=mode)
                        obfuscated_grads.append(go)
                    input_gradient = tuple(obfuscated_grads)

                    # Apply additive noise
                    noise_std = defense_setting.get('noise', 0.01)
                    input_gradient = apply_additive_noise(input_gradient, std=noise_std)

                else:
                    raise ValueError(f"Unsupported defense method: {config['defense_method']}. Choose 'diff2obs' or None.")

                time_defense = time.time() - start
                print(f"Time defense: {time_defense:.6f} seconds")
                ...
                if G is None:
                    G = rec_machine.G
                logger.info(f"Real labels:{labels}")
                ...
                input_parameters = inversefed.reconstruction_algorithms.loss_steps(
                    model, ground_truth, labels, lr=local_lr, local_steps=local_gradient_steps, use_updates=True, batch_size=batch_size
                )
                input_parameters = [p.detach() for p in input_parameters]

                rec_machine = inversefed.FedAvgReconstructor(
                    model, (dm, ds), local_gradient_steps, local_lr, config_m, num_images=config['num_images'], use_updates=True, batch_size=batch_size
                )
                if G is None:
            ...
            for idx, item in enumerate(result):
                ...
                if output is None or output.shape[-1] != ground_truth.shape[-1]:
                    test_psnr = -1
                    lpips_score = -1
                    lpips_score_a = -1
                    ssim_score = -1
                    ...
                    logger.info(f"output_den's dimension:{output_den.shape}")
                ...
                inversefed.utils.save_to_table(
                    os.path.join(ouput_dir), name=f'{exp_name}', dryrun=args.dryrun,
                    rec_loss=stats["opt"],
                    psnr=test_psnr,
        ...
        logger.info(f'Best noise gradient L2 norm: {noisy_input_gradient_norm}')
